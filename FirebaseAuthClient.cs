    ...
    public class FirebaseAuthClient : IFirebaseAuthClient
    {
        ...
        public event EventHandler<UserEventArgs> AuthStateChanged
        {
            add
            {
                ...
                // Trigger the AuthStateChanged event in a background task
                TriggerAuthStateChangedAsync(value);
            }
            remove
            {
                this.authStateChanged -= value;
        ...
        private async void TriggerAuthStateChangedAsync(EventHandler<UserEventArgs> handler)
        {
            try
            {
                var user = await this.config.UserManager.GetUser();
                ...
                if (user.info == null)
                {
                    this.TriggerAuthStateChanged(handler, null);
                }
                else
                {
                    this.TriggerAuthStateChanged(handler, new User(this.config, user.info, user.credential));
                }
            }
            catch (Exception ex)
            {
                // Handle or log exceptions as needed
                Console.WriteLine($"Error in TriggerAuthStateChangedAsync: {ex.Message}");
            }
        }
        ...
        public async Task<UserCredential> SignInWithRedirectAsync(FirebaseProviderType authType, SignInRedirectDelegate redirectDelegate)
        {
            ...
            await this.SaveToken(userCredential.User);
        ...
        public async Task SignOut()
        {
            ...
            this.User = null;
            await this.config.UserManager.DeleteExistingUser(uid);
        }
        ...
        private async Task SaveToken(User user)
        {
            await this.config.UserManager.SaveNewUser(user);
        }
